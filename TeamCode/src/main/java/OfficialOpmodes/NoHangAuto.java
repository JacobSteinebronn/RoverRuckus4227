...
@Autonomous(name = "NoHangAuto", group = "Auto")
public class NoHangAuto extends LinearOpMode {
...
//                        Thread.sleep(500);
    ...
    private void scanSampleState() throws InterruptedException {
        ...
        if(System.currentTimeMillis()-lastTimeLogged>200){
        ...
        if(detector.getAligned()||detector.getXPosition()<400&&detector.getXPosition()>10) {
            success++;
            Log.i("hhs4227",String.format("x: %f, Seen, aligned: "+detector.getAligned(), detector.getXPosition()));
            robot.motorL.setPower(0);
            robot.motorR.setPower(0);
            Thread.sleep(100);
        }

        if(success>3||heading<-45){
            detector.disable();
